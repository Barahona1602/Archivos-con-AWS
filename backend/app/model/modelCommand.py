...
class modelCommand(BaseModel):
  command: str = "create -name->prueba1.txt -path->/carpeta1/ -body->'Este es el contenido del archivo 1' -type->server"
...
Backup -type_to->bucket -type_from->bucket -ip->3.144.137.114 -name->'copia_1 G7' -port->3000

Recovery -type_to->server -type_from->bucket -name->'copia_1 G7'
Recovery -type_to->bucket -type_from->bucket -ip->3.144.137.114 -name->'copia_1 G7' -port->3000

Delete_all -type->server

open -type->server -name->archivo1.txt
open -type->bucket -ip->3.144.137.114 -name->'archivo 1.txt' -port->3000
"""